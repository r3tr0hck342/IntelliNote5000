...
[ -n "$GEMINI_API_KEY" ] && echo "✅ GEMINI_API_KEY set" || echo "❌ No GEMINI_API_KEY set"
...
jq -e '.package.productName and .package.identifier' src-tauri/tauri.conf.json >/dev/null 2>&1 && echo "✅ tauri.conf.json metadata set" || echo "❌ Missing tauri.conf.json fields"
...
[ -f .env.local ] && echo "✅ .env.local exists (copied from .env.example)" || echo "❌ .env.local not created"
grep -q VITE_API_KEY .env.local && echo "✅ VITE_API_KEY present in .env.local" || echo "❌ VITE_API_KEY missing in .env.local"

npm run dev >/dev/null 2>&1 && echo "✅ npm run dev succeeded" || echo "❌ npm run dev failed"
...
echo "- [$(test -f .env.local && echo 'x' || echo ' ')] .env.local present"
echo "- [$(grep -q VITE_API_KEY .env.local 2>/dev/null && echo 'x' || echo ' ')] VITE_API_KEY present"
...
echo "- [$(jq -e '.package.productName and .package.identifier' src-tauri/tauri.conf.json >/dev/null 2>&1 && echo 'x' || echo ' ')] tauri.conf.json metadata"
